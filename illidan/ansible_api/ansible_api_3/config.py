...
__all__ = ['Config']
FILE_REAL_PATH = os.path.dirname(os.path.realpath(__file__))
...
class Config(object):
    ...
    log_path = FILE_REAL_PATH + os.sep + "ansible_api.log"
    allow_ip = []
    thread_pool_size = 4
    ...
    def __init__(self):
        cfg_path = FILE_REAL_PATH + os.sep + "ansible_api.cfg"
        ...
        try:
            ...
            if cf.has_option('default', 'log_path'):
                self.log_path = cf.get('default', 'log_path') if cf.get('default', 'log_path') else self.log_path
